...
{
    return std::make_shared<Monster>("Scorpion", positionX, positionY, 64, 64, 100, 100, animation);
}

std::shared_ptr<Monster> GameObjectFactory::createFirebug(
    float positionX, float positionY, Animation& animation)
{
    return std::make_shared<Monster>("Firebug", positionX, positionY, 64, 64, 200, 50, animation);
}