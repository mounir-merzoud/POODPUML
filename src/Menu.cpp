...
void afficherFenetreJeu() {
    ...
    if (!textureJeu.loadFromFile("assets/menu/BKG.png")) {
    ...
    sf::Font police;
    if (!police.loadFromFile("assets/menu/Supermassive Black Hole - TTF.ttf")) {
...
void afficherFenetreOptions(bool& musiqueActivee, sf::Music& musique) {
    ...
    if (!textureIconeActiver.loadFromFile("assets/menu/la-musique (1).png")) {
        std::cerr << "Erreur de chargement de l'icone d'activation\n";
        return;
    }

    sf::Texture textureIconeDesactiver;
    if (!textureIconeDesactiver.loadFromFile("assets/menu/la-musique.png")) {
...
int main() {
    ...
    sf::Music musique;
    if (!musique.openFromFile("assets/menu/pixel-fight-8-bit-arcade-music-background-music-for-video-208775.mp3")) {
    ...
    sf::Texture texture;
    if (!texture.loadFromFile("assets/menu/2x1_NSwitchDS_FantasyTowerDefense_image1600w.jpg")) {
